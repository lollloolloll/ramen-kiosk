  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const onEditSubmit = async (data: RamenFormData) => {
        ...
        const result = await updateRamen(row.original.id, formData);
        if (result?.success) {
      ...
      const onDelete = async () => {
        const result = await deleteRamen(row.original.id);
        if (result?.success) {
        ...
        <div className="flex space-x-2">
          ...
          >
            ...
            <DialogContent>
              ...
              <p>Are you sure you want to delete "{row.original.name}"?</p>
...
export default async function AdminStockPage() {
  const ramenList: Ramen[] = await db.query.ramens.findMany();
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <DataTable columns={columns} data={ramenList} />
    </div>
  );
}
